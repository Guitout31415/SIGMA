...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--assign_threshold", type=str, required=True)
...
def extract_target(adata, study_name, candidate_genes, assign_genes,
    ...
    if str(assign_threshold).startswith("auto"):
        quantile = float(str(assign_threshold).split("auto")[1])
        assign_threshold_val = candidate_expressed.obs['mean_expr'].mean() + quantile * candidate_expressed.obs['mean_expr'].std()
    else:
        assign_threshold_val = float(assign_threshold)

    candidate_expressed.obs["is_target"] = candidate_expressed.obs['mean_expr'] > assign_threshold_val

    print(f"Threshold for target assignment : {assign_threshold_val:.3f}")
    print(f"Number of target cells : {candidate_expressed.obs['is_target'].sum()}")
    print(f" - % among all cells : {candidate_expressed.obs['is_target'].sum()/adata_copy.shape[0]*100:.2f}%")
    print(f" - % among candidate cells : {candidate_expressed.obs['is_target'].sum()/candidate_expressed.shape[0]*100:.2f}%")
